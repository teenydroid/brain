---
name: humanizer
description: Rewrite AI-sounding text so it reads like the writer, not a chatbot. Apply to every post, reply, and draft before publishing.
category: Voice
---
...
Before returning the rewrite, search for the em dash and en dash characters. Remove each one unless the writer's sample uses that mark. In that case, match the sample's rate.
...
**Pasted text (default).** Return the rewritten text only. No preamble, no summary of what changed, no offer to keep going. If the user asks what changed, list the pattern numbers you applied.

**A file.** Rewrite the file in place and report which patterns you applied, by number.

**A draft post or reply.** Return the final text, ready to publish, and nothing else. Check it against the account's hard formatting rules before returning it.

**Nothing to fix.** Say so in one line rather than rewriting good text into different text.
